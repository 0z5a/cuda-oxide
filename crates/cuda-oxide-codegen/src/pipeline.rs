...
use crate::cutlass_backend::{CutlassBackendConfig, compile_to_cubin};
...
use cuda_oxide_mlir_export::{CutlassFullCuteMlir22, ExportManifest, MlirConsumerProfile};
...
use pliron_mlir_export::render_module as render_mlir_module;
...
pub struct OutputFiles<'a> {
    ...
    pub ptx: &'a Path,
    /// Cubin output. This is populated only by the selected CUTLASS backend.
    pub cubin: &'a Path,
    /// Optional textual MLIR observation of the prepared semantic module.
    pub mlir: Option<&'a Path>,
...
pub struct ModulePipelineRequest<'a> {
    ...
    cutlass_backend: Option<&'a CutlassBackendConfig>,
    expected_kernels: &'a [String],
}
...
pub enum ModuleArtifactKind {
    ...
    NvvmIr,
    /// Final cubin generated by the official CUTLASS compiler library.
    Cubin,
}
...
pub fn compile_translated_module(
    ...
    if let Some(path) = request.files.mlir {
        validate_cutlass_mlir_output_path(path, &request.files)?;
    }
    if let Some(config) = request.cutlass_backend {
        validate_cutlass_compile_policy(request.backend, request.debug_kind)?;
        if matches!(
            ...
            return Err(PipelineError::BackendB(
                "CUDA_OXIDE_EMIT_NVVM_IR cannot be combined with CUDA_OXIDE_DEVICE_BACKEND=cutlass-mlir; the CUTLASS backend already produces a final cubin"
                    .to_owned(),
            ));
        }
        let rendered = cutlass_rendered
            .as_ref()
            .expect("the selected CUTLASS backend always renders its input");
        remove_stale_files(request.files.stale_before_export)?;
        if request.trace.verbose {
            request.trace.emit(format!(
                "\n=== Running official CUTLASS compiler → {} ===",
                request.files.cubin.display()
            ));
        }
        let generated = compile_to_cubin(
            config,
            &rendered.text,
            &rendered.target,
            request.expected_kernels,
            request.files.cubin,
            request.files.mlir,
        )?;
        ...
        if request.trace.verbose {
            if let Some(path) = request.files.mlir {
                request
                    .trace
                    .emit(format!("CUTLASS MLIR written to {}", path.display()));
            }
            request.trace.emit(format!(
                "✓ cubin written to {} (target: {})",
                request.files.cubin.display(),
                rendered.target
            ));
        }
        return Ok(ModulePipelineOutput {
            artifact_kind: ModuleArtifactKind::Cubin,
            target: rendered.target.clone(),
            diagnostics: generated.diagnostics,
        });
    }
    if let (Some(path), Some(rendered)) = (request.files.mlir, cutlass_rendered.as_ref()) {
        std::fs::write(path, &rendered.text).map_err(|error| {
            PipelineError::Export(format!(
                "could not write requested CUTLASS MLIR to {}: {error}",
                path.display()
            ))
        })?;
        if request.trace.verbose {
...
fn validate_cutlass_compile_policy(
    backend: &BackendOptions,
    debug_kind: DebugKind,
) -> Result<(), PipelineError> {
    if backend.no_fma {
        return Err(PipelineError::BackendB(
            "CUDA_OXIDE_NO_FMA/--no-fmad is not implemented by the pinned CUTLASS backend; refusing to compile with different floating-point contraction semantics"
                .to_owned(),
        ));
    }
    if backend.no_opt {
        return Err(PipelineError::BackendB(
            "unoptimized device code is not implemented by the pinned CUTLASS backend; refusing to silently use its fixed optimized pipeline"
                .to_owned(),
        ));
    }
    if debug_kind != DebugKind::Off {
        return Err(PipelineError::BackendB(
            "device debug information is not implemented by the pinned CUTLASS backend; use the native backend for line tables or variable debug information"
                .to_owned(),
        ));
    }
    Ok(())
...
fn validate_cutlass_mlir_output_path(
    output: &Path,
    files: &OutputFiles<'_>,
) -> Result<(), PipelineError> {
    let normalized_output = normalize_artifact_path(output).map_err(|error| {
        PipelineError::Export(format!(
            "could not safely normalize requested CUTLASS MLIR path {}: {error}",
            output.display()
        ))
    })?;
    for native in std::iter::once(files.llvm_ir)
        .chain(std::iter::once(files.ptx))
        .chain(std::iter::once(files.cubin))
        .chain(files.stale_before_export.iter().map(PathBuf::as_path))
    {
        let normalized_native = normalize_artifact_path(native).map_err(|error| {
            PipelineError::Export(format!(
                "could not safely normalize ordinary compiler artifact path {} while checking CUTLASS MLIR output {}: {error}",
                native.display(),
                output.display()
            ))
        })?;
        if normalized_output == normalized_native {
            return Err(PipelineError::Export(format!(
                "requested CUTLASS MLIR path {} collides with ordinary compiler artifact {}",
                output.display(),
                native.display()
            )));
        }
...
/// Resolve every existing path component, then normalize the remaining tail.
///
/// `canonicalize` alone cannot handle a not-yet-created output file. Walking
/// upward to the first resolvable ancestor preserves symlink resolution for
/// every existing directory while still giving new sibling artifacts a stable
/// absolute identity. A broken symlink is an error rather than an ordinary
/// missing component: treating it as text could let the later write follow a
/// path the collision check never inspected.
fn normalize_artifact_path(path: &Path) -> io::Result<PathBuf> {
            ...
            Err(error) if error.kind() == io::ErrorKind::NotFound => {
                ...
                let component = probe.components().next_back().ok_or_else(|| {
                    io::Error::new(
                        ...
                        format!("path {} has no resolvable ancestor", path.display()),
                    )
                })?;
                unresolved.push(component.as_os_str().to_os_string());
                if !probe.pop() {
    ...
    normalize_absolute_lexically(&resolved)
}

fn normalize_absolute_lexically(path: &Path) -> io::Result<PathBuf> {
    ...
    if !normalized.is_absolute() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
                "normalized artifact path is not absolute: {}",
                path.display()
            ),
        ));
    }
...
struct RenderedCutlassMlir {
    text: String,
    target: String,
}

fn render_live_cutlass_mlir(
    ctx: &Context,
    module: Ptr<Operation>,
    backend: &BackendOptions,
) -> Result<RenderedCutlassMlir, PipelineError> {
    let gpu_arch = backend
        .target_arch
        .as_deref()
        .or(backend.device_arch_hint.as_deref())
        .ok_or_else(|| {
            PipelineError::Export(
                "CUTLASS MLIR was requested, but no GPU architecture is available; set \
                 CUDA_OXIDE_TARGET to a profile-supported target"
                    .to_owned(),
            )
        })?;
    let profile = CutlassFullCuteMlir22::new(gpu_arch).map_err(|error| {
        PipelineError::Export(format!(
            "CUTLASS MLIR profile selection failed for `{gpu_arch}`: {error}"
        ))
    })?;
    let source = Operation::get_op::<ModuleOp>(module, ctx).ok_or_else(|| {
        PipelineError::Export(
            "CUTLASS MLIR export expected the live source operation to be builtin.module"
                .to_owned(),
        )
    })?;
    let translated = profile.translate_module(ctx, &source).map_err(|error| {
        PipelineError::Export(format!("CUTLASS MLIR translation failed: {error}"))
    })?;
    let text = render_cutlass_mlir(&profile.manifest(), &translated)?;
    Ok(RenderedCutlassMlir {
        text,
        target: gpu_arch.to_owned(),
    })
}

fn render_cutlass_mlir(
    manifest: &ExportManifest,
    module: &pliron_mlir_export::MlirModule,
) -> Result<String, PipelineError> {
    let manifest = manifest.to_pretty_json().map_err(|error| {
        PipelineError::Export(format!(
            "could not render the CUTLASS profile manifest: {error}"
        ))
    })?;
    let mut output = String::from("// cuda-oxide CUTLASS export manifest\n");
    for line in manifest.lines() {
        output.push_str("// ");
        output.push_str(line);
        output.push('\n');
    }
    output.push_str(&render_mlir_module(module));
    Ok(output)
}
...
mod tests {
    ...
    #[test]
    fn cutlass_policy_rejects_unimplemented_fma_optimization_and_debug_modes() {
        let mut backend = BackendOptions::default();
        assert!(validate_cutlass_compile_policy(&backend, DebugKind::Off).is_ok());

        backend.no_fma = true;
        let error = validate_cutlass_compile_policy(&backend, DebugKind::Off).unwrap_err();
        assert!(error.to_string().contains("--no-fmad"), "{error}");

        backend.no_fma = false;
        backend.no_opt = true;
        let error = validate_cutlass_compile_policy(&backend, DebugKind::Off).unwrap_err();
        assert!(error.to_string().contains("unoptimized"), "{error}");

        backend.no_opt = false;
        let error = validate_cutlass_compile_policy(&backend, DebugKind::LineTables).unwrap_err();
        assert!(error.to_string().contains("debug information"), "{error}");
    }

    #[test]
    fn cutlass_output_starts_with_a_comment_only_pinned_manifest() {
        let manifest = ExportManifest {
            profile: "test-profile".to_owned(),
            exporter_version: "0.1.0".to_owned(),
            pliron_revision: "pliron-rev".to_owned(),
            consumer_revision: "consumer-rev".to_owned(),
            llvm_major: 22,
            gpu_arch: "sm_120a".to_owned(),
            pointer_model: "opaque".to_owned(),
            index_bits: 64,
        };
        let module = pliron_mlir_export::MlirModule {
            root: pliron_mlir_export::MlirOperation::new("builtin.module").unwrap(),
            profile: "test-profile".to_owned(),
        };

        let text = render_cutlass_mlir(&manifest, &module).unwrap();
        let module_start = text.find("\"builtin.module\"").unwrap();
        assert!(text.starts_with("// cuda-oxide CUTLASS export manifest\n// {\n"));
        assert!(text.contains("//   \"consumer_revision\": \"consumer-rev\",\n"));
        assert!(
            text[..module_start]
                .lines()
    ...
    #[test]
    fn cutlass_output_must_not_be_overwritten_by_native_artifacts() {
        let ll = Path::new("kernel.ll");
        let ptx = Path::new("kernel.ptx");
        let stale = [PathBuf::from("kernel.cubin")];
        let files = OutputFiles {
            llvm_ir: ll,
            ptx,
            cubin: &stale[0],
            mlir: Some(Path::new("kernel.mlir")),
            stale_before_export: &stale,
        };

        assert!(validate_cutlass_mlir_output_path(Path::new("kernel.mlir"), &files).is_ok());
        for collision in [ll, ptx, stale[0].as_path()] {
            let error = validate_cutlass_mlir_output_path(collision, &files).unwrap_err();
            assert!(error.to_string().contains("collides"));
        }
    }

    #[test]
    fn cutlass_relative_output_cannot_alias_an_absolute_native_artifact() {
        let current = std::env::current_dir().unwrap();
        let relative = Path::new("cuda-oxide-mlir-collision-check.ll");
        let ll = current.join(relative);
        let ptx = current.join("cuda-oxide-mlir-collision-check.ptx");
        let cubin = current.join("cuda-oxide-mlir-collision-check.cubin");
        let files = OutputFiles {
            llvm_ir: &ll,
            ptx: &ptx,
            cubin: &cubin,
        ...
        let error = validate_cutlass_mlir_output_path(relative, &files).unwrap_err();
        assert!(error.to_string().contains("collides"));
    }

    #[test]
    fn cutlass_output_cannot_alias_a_native_artifact_through_parent_components() {
        let root = tempfile::tempdir().unwrap();
        let child = root.path().join("child");
        std::fs::create_dir(&child).unwrap();
        let ll = root.path().join("kernel.ll");
        let ptx = root.path().join("kernel.ptx");
        let cubin = root.path().join("kernel.cubin");
        let alias = child.join("..").join("kernel.ll");
        let files = OutputFiles {
            llvm_ir: &ll,
            ptx: &ptx,
            cubin: &cubin,
        ...
        let error = validate_cutlass_mlir_output_path(&alias, &files).unwrap_err();
        assert!(error.to_string().contains("collides"));
    }

    #[cfg(unix)]
    #[test]
    fn cutlass_output_resolves_symlinked_parents_before_comparison() {
        ...
        symlink(&real, &alias_dir).unwrap();
        let ll = real.join("kernel.ll");
        let ptx = real.join("kernel.ptx");
        let cubin = real.join("kernel.cubin");
        let alias = alias_dir.join("kernel.ll");
        let files = OutputFiles {
            llvm_ir: &ll,
            ptx: &ptx,
            cubin: &cubin,
    ...
    #[cfg(unix)]
    #[test]
    fn cutlass_output_fails_closed_on_a_broken_symlink() {
        use std::os::unix::fs::symlink;

        let root = tempfile::tempdir().unwrap();
        let broken = root.path().join("backend.mlir");
        ...
        let error = validate_cutlass_mlir_output_path(&broken, &files).unwrap_err();
        assert!(error.to_string().contains("could not safely normalize"));
    }
